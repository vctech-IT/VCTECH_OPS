<!-- routes/deliveryChallans/+page.svelte -->
<script lang="ts">
  ...
  let selectedStatuses = data.sortColumn === 'status' ? (data.status || '').split(',') : [];
  ...
  // Navigate to Delivery Challan Details
  function navigateToDeliveryChallanDetails(dcNumber_id: string) {
    goto(`/deliveryChallan/${dcNumber_id}`);
  }

  // Pagination Functions
  function getPaginationRange() {
    const totalPages = Math.ceil(totalDeliveryChallans / itemsPerPage);
  ...
  $: totalDeliveryChallans = data.totalDeliveryChallans;
  $: availableStatuses = data.availableStatuses;
  $: availableBranches = data.availableBranches;
  $: currentPage = data.page;
  $: itemsPerPage = data.limit;
  $: error = data.error;
  $: totalPages = Math.ceil(totalDeliveryChallans / itemsPerPage);
...
<div class="container mx-auto px-4 py-8 max-w-7xl">
  ...
  <div class="mb-4 flex justify-between items-center">
    <div class="relative w-full mr-4">
      <input
        type="text"
        placeholder="Search delivery challans by customer, DC number, or reference..."
  ...
  {#if deliveryChallans.length === 0}
    <div class="text-center py-8 text-gray-500">
      <p>No delivery challans found. Try a different search term or filter.</p>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-blue-500 text-white">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" on:click={() => toggleSort('dcNumber')}>
              <div class="flex items-center">
                DC #
                {#if sortColumn === 'dcNumber'}
                  {#if sortDirection === 'asc'}
                    <ArrowUp size={14} class="ml-1" />
                  {:else}
                    <ArrowDown size={14} class="ml-1" />
            ...
            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" on:click={() => toggleSort('customerName')}>
              <div class="flex items-center">
                Customer
            ...
            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" on:click={() => toggleSort('branchName')}>
              <div class="flex items-center">
                Branch
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          {#each deliveryChallans as challan (challan.id)}
            <tr 
              class="hover:bg-blue-50 cursor-pointer transition-colors duration-200"
              on:click={() => navigateToDeliveryChallanDetails(challan.dcNumber_id)}
              transition:fade
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {getStatusClasses(challan.status)}">
                  {challan.status}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                ₹{challan.total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </td>
            </tr>
          {/each}
        </tbody>
    ...
    <div class="flex justify-between items-center mt-4">
      <div class="flex items-center text-sm text-gray-600">
        <span>
          ...
          {Math.min(currentPage * itemsPerPage, totalDeliveryChallans)} 
          of {totalDeliveryChallans} delivery challans
        </span>
  ...
  {#if isFilterModalOpen}
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" transition:fade>
      <div class="bg-white p-6 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
        <h2 class="text-xl font-bold mb-4">Filter Delivery Challans</h2>
